...
- **État des branches au moment du handoff** :
  ...
  - `refactor` = **`fa25fb3`** = `main` + **LanesMatchView** (`06b6206`) + **OnlinePage** (`9e31299` rename incomplet → complété `fa25fb3`) — **2 splits EN ATTENTE device-test** (cadence : Alex a demandé « build après le tout » → on teste/merge les 2 ensemble).
    - LanesMatchView ✅ PAS d'exception : JSX-dense → présentationnel pur (12 fichiers), orchestrateur **235 l (<400)**.
    - OnlinePage ⚠️ **2e EXCEPTION ASSUMÉE** : `feasibleUnder600=false` (cœur sync WebSocket onMessage/refs-miroir/watchdog/bot-fallback non extractible verbatim). Passe verbatim sûre → orchestrateur **1340 l** (au-dessus de 600), présentationnel (8 feuilles <400 : MenuPickers/StatusAndWaiting/MatchFlowSplash/MatchFlowRound/MatchEndScene/QueueRadar) + `useServerStatus` + types extraits. Cœur intact via `git mv`. Diff verbatim le plus propre de la chaîne ([A] = 4 lignes d'import). ⚠️ split en 2 commits (`9e31299` rename + `fa25fb3` complétion ; même piège git-add-post-mv que RankedGame).

> **🎉 CHAÎNE DE REFACTOR TERMINÉE** — tous les god-components (>600 l) ont été splittés. Restent au-dessus du plafond UNIQUEMENT : les 2 exceptions assumées (RankedGame 1341 mergé, OnlinePage 1340 sur refactor) — cœurs de match/sync logique-denses, non extractibles verbatim sans réécriture risquée ; `cards.ts` (skip, piège gen-card-meta) ; les locales i18n (data, exemptes) ; `App.tsx` (605, shell racine, à évaluer séparément si besoin). Lot suivant éventuel pour les exceptions = hooks (useRankedMatch / useOnlineConnection) + device-test lourd — HORS chaîne verbatim.
...
| ~~`pages/OnlinePage.tsx`~~ | ~~2575~~ | — | ✅ **FAIT (2e EXCEPTION)** (`9e31299`+`fa25fb3`, 10 fichiers). Analyse multi-agents → mixed, `feasibleUnder600=false`. Passe verbatim sûre : 8 feuilles <400 + useServerStatus + types extraits, orchestrateur sync **1340 l** (exception, cœur WebSocket intact via `git mv`). Diff [A]=4 imports. EN ATTENTE device-test. |
...
- **2026-06-15/16 : 6 splits MERGÉS + DÉPLOYÉS** → `main` = `41c74b2`, `/health` 200 (ProfilePage/PlayGame/ArenaLaneSlot/ArenaBoard/ArenaGame puis RankedGame, device-test « fonctionnel » Alex avant chaque merge ; APK device installée).
- `refactor` (`fa25fb3`) porte **LanesMatchView + OnlinePage** (les 2 derniers), EN ATTENTE device-test groupé (Alex : « build après le tout »). Après test OK → FF-merge `refactor → develop → main` + push → **Render redéploie** depuis `main`.
  ⚠️ **PIÈGE git récurrent** (RankedGame ET OnlinePage) : après `git mv X.tsx dossier/X.tsx`, faire `git add X.tsx dossier/` ÉCHOUE (pathspec ancien chemin) et avorte tout → le commit ne capte que le rename. Faire `git add dossier/` SEUL, vérifier `git status` AVANT de commit. Les 2 splits sont donc en 2 commits (rename incomplet + complétion) ; le TIP est correct, les commits-rename intermédiaires seuls ne compilent pas.
